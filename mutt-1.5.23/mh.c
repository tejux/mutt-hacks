...
int maildir_check_mailbox (CONTEXT * ctx, int *index_hint)
{
        ...
        char buf[_POSIX_PATH_MAX];
        int changed = 0;
/* bitmask representing which subdirectories
        ...
        int i;
        HASH *fnames;
/* hash table for quickly looking up the base filename
...
FILE *maildir_open_find_message (const char *folder, const char *msg)
{
        ...
        FILE *fp;
                                                  ...
                                                  /* simple dynamic optimization */
        static unsigned int new_hits = 0, cur_hits = 0;
