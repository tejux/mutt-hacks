...
BODY *pgp_gpgme_encrypt_message (BODY *a, char *keylist, int sign)
{
        ...
        t->parts->next->d_filename = safe_strdup ("msg.asc");
/* non pgp/mime
                                                           can save */
   ...
   -moritz.  */
                ...
                if (gpg_err_code (sig->status) != GPG_ERR_NO_PUBKEY) {
                        ...
                        if (! err) {
                                ...
                                key = NULL;
/* Old gpgme versions did not set KEY to NULL on
...
/*      key = pkey; */
        ...
        kflags = (key->flags
/*| (pkey->flags & KEYFLAG_RESTRICTIONS)
                                         | uid->flags*/
                );                                        
...
#endif
                has_run = 1;
/* note use of 1 here is intentional to avoid requiring "true"
