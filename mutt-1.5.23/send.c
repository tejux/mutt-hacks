...
/* Normally, this loop will not even be entered. */
                                for (t = from; t && t->next; t = t->next)
                                        ;
                                                  ...
                                                  /* t cannot be NULL at this point. */
                                t->next = e->mail_followup_to;
...
/* set the default FCC */
                if (!msg->env->from) {
                        msg->env->from = mutt_default_from ();
                        killfrom = 1;
/* no need to check $use_from because if the user specified
 ...
 */
                        ...
                        mutt_prepare_envelope (msg->env, 0);
                                                  /* Handle bad IDNAs the next time. */
                        mutt_env_to_idna (msg->env, NULL, NULL);
                ...
                if (!(flags & SENDBATCH)) {
                        ...
                        && msg->content->type == TYPEAPPLICATION)) {
                                ...
                                msg->content = clear_content;
                                                  ...
                                                  /* destroy sig */
                                mutt_free_body (&msg->content->parts->next);
                                msg->content = mutt_remove_multipart (msg->content);
                        }
