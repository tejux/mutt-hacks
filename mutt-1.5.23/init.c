...
static void pretty_var (char *dst, size_t len, const char *option, const char *val)
{
        ...
        if (p - dst < len)
                *p++ = '"';
                                                  /* \0 terminate it */
        p += escape_string (p, len - (p - dst) + 1, val);
        if (p - dst < len)
                *p++ = '"';
        *p = 0;
}
...
static char Completed [STRING] =                  /* completed string (command or variable) */
{
        0
};
...
/* some systems report the FQDN instead of just the hostname */
        if ((p = strchr (utsname.nodename, '.'))) {
                ...
                p++;
                                                  /* save the domain for below */
                strfcpy (buffer, p, sizeof (buffer));
        }
        else
