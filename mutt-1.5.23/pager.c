 ...
 * line's color as default. */
                        if (n > 0 && (buf[0] == ' ' || buf[0] == '\t')) {
                                                  /* wrapped line */
                                lineInfo[n].type = lineInfo[n-1].type;
...
/* now adjust the first line of this header field */
                                                if (j >= 0) {
                                                  ...
                                                  /* the previous lines have already been drawn on the screen */
                                                *force_redraw = 1;
                                        }
                                        break;
                                }
                                                  ...
                                                  /* a little hack... */
        else if (mutt_strncmp ("\033[0m", raw, 4) == 0)
                lineInfo[n].type = MT_COLOR_NORMAL;
#if 0
                                ...
                                (offset ? REG_NOTBOL : 0)) == 0) {
                                        if (pmatch[0].rm_eo != pmatch[0].rm_so) {
                                                ...
                                                null_rx = 0;
                                                  ...
                                                  /* empty regexp; don't add it, but keep looking */
                                                null_rx = 1;
                                }
...
mutt_pager (const char *banner, const char *fname, int flags, pager_t *extra)
{
        ...
        int SearchCompiled = 0, SearchFlag = 0, SearchBack = 0;
                                                  /* main message or rfc822 attachment */
        int has_types = (IsHeader(extra) || (flags & M_SHOWCOLOR)) ? M_TYPES : 0;
