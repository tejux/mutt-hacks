...
{
                                                  /* ! not included because it can be part of a pathname in Mutt */
        static const wchar_t shell_chars[] = L"<>&()$?*;{}| ";
...
/* skip over one non-alphanumeric character */
                                ...
                                case OP_EDITOR_BUFFY_CYCLE:
                                        if (flags & M_EFILE) {
                                                  /* clear input if user types a real key later */
                                                first = 1;
...
/* file selection cancelled */
                                                ...
                                                if (!mutt_complete (buf, buflen)) {
                                                  ...
                                                  /* let the user know that nothing matched */
                                                                BEEP ();
                                                replace_part (state, 0, buf);
                                        }
                                        else
