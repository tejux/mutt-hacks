...
static char LastSearch[STRING] =                  /* last pattern searched for */
{
        0
};
...
/* range minimum */
                if (*s->dptr == '>') {
                        pat->max = M_MAXRANGE;
                                                  /* exclusive range */
                        pat->min = strtol (s->dptr + 1, &tmp, 0) + 1;
                }
                else
        ...
        for ( ; n ; n --) {
                for (a = va_arg (ap, ADDRESS *) ; a ; a = a->next) {
                        ...
                        (match_personal && a->personal && patmatch (pat, a->personal) == 0))) {
                                va_end (ap);
                                                  /* Found match, or non-match if alladdr */
                                return (! pat->alladdr);
                        }
                }
        }
